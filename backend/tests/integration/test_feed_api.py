...
class TestFeedAPI:
    ...
    def test_get_feed_happy_path_200(self, mock_tx_find, mock_membership):
        """Позитивний сценарій: Юзер є в групі, отримуємо стрічку транзакцій (200 OK)"""
        # 1. Імітуємо, що юзер є в групі
        mock_membership.return_value = True

        # 2. Імітуємо складний ланцюжок Beanie: find().sort().skip().limit().to_list()
        mock_find_query = MagicMock()
        mock_find_query.count = AsyncMock(return_value=1)  # Всього 1 транзакція
    ...
    def test_get_feed_forbidden_403(self, mock_membership):
        ...
        # Імітуємо відсутність юзера в групі
        mock_membership.return_value = None

        res = client.get("/api/v1/feed/607f1f77bcf86cd799439033")

        assert res.status_code == 403
        assert res.json()["message"] == "Ви не є учасником цієї групи"
    ...
    def test_get_feed_invalid_group_id_400(self):
        ...
        assert "Невалідний ID групи" in res.json()["message"]
